ruleset wovyn_base {
  ...
  rule threshold_notification {
    ...
    event:send(
      {"eci": sensor_subs{"Tx"}, "eid":"from wovyn violation", "domain":"manage", "type":"threshold_violation",
        "attrs": event:attrs
      })
  }
  ...
  rule get_report {
    ...
    pre {
      eci = event:attr("return_eci").klog()
      id = event:attr("correlation_id").klog()
    }
    event:send(
      {"eci":eci, "eid": "from_" + meta:eci + "report", "domain": "sensor", "type": "return_report",
        "attrs": {"correlation_id": id, "report_info": temperature_store:temperatures(), "Tx": meta:eci}
      })
